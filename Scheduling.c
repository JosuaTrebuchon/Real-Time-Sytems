...
void shortest_remaining_time(struct Task ** tasks, int taskCount, int timeout, int interval){
	...
	while(1){
		...
		while(counter < taskCount){
			...
			if(tasks[counter]->state != finished 
				&& (tasks[counter]->totalRuntime - tasks[counter]->currentRuntime) < shortestTime 
				&& (tasks[counter]->startTime <= totalTimePassed) ) {
		...
		if(tasks[currentTaskIndex]->startTime > totalTimePassed || counter_finished == taskCount){ // counter >= taskCount
			totalTimePassed ++; //Speeding through areas without tasks to schedule
		} else {
			if(prevTaskIndex != currentTaskIndex && prevTaskIndex != -1 && tasks[prevTaskIndex]->state != finished){ // prevTaskIndex == -1 means that this is the first task to be selected
				set_task_state(tasks[prevTaskIndex], preempted);
...
void feedback(struct Task ** tasks, int taskCount, int timeout, int interval, int** priority_queue){
	...
	while(1){
		...
		if(queue_counter >= 6){ // counter >= taskCount
			totalTimePassed ++; //Speeding through areas without tasks to schedule
			finished = 1;
		} else {
...
void feedback2(struct Task ** tasks, int taskCount, int timeout, int interval, int** priority_queue){
	printf("Started shortest_remaining_time scheduler with %d tasks \n", taskCount);
	fflush(stdout);
	int totalTimePassed = 0;
	int currentTaskIndex = 0;
	...
	while(1){
		...
		while(counter < taskCount){
			currentTaskIndex = (currentTaskIndex + 1) % taskCount;
			if(priority_queue[queue_counter][queue_index]  >= 0){
				break;
			}
			counter ++;
		}
...
int main(){
	...
	for(int i = 0; i < 7; i++)
	{	
		priority_queue[i] = (int*) malloc(5*sizeof(int));
	...
	feedback2(tasks, taskCount, schedulerTimeout, (int) 10, priority_queue);
